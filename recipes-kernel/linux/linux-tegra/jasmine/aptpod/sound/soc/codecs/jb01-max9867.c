...
static const struct snd_kcontrol_new max9867_left_input_mixer_controls[] = {
	SOC_DAPM_SINGLE("Line Left Capture Switch", MAX9867_INPUTCONFIG, 7, 1, 0),
	SOC_DAPM_SINGLE("Mic Left Capture Switch", MAX9867_INPUTCONFIG, 6, 1, 0)
};

static const struct snd_kcontrol_new max9867_right_input_mixer_controls[] = {
	SOC_DAPM_SINGLE("Line Right Capture Switch", MAX9867_INPUTCONFIG, 5, 1, 0),
	SOC_DAPM_SINGLE("Mic Right Capture Switch", MAX9867_INPUTCONFIG, 4, 1, 0)
};

/* Output mixer */
static const struct snd_kcontrol_new max9867_left_output_mixer_controls[] = {
	SOC_DAPM_SINGLE("Line Left Bypass Switch", MAX9867_LEFTLINELVL, 6, 1, 1),
};

static const struct snd_kcontrol_new max9867_right_output_mixer_controls[] = {
	SOC_DAPM_SINGLE("Line Right Bypass Switch", MAX9867_RIGHTLINELVL, 6, 1, 1),
};

/* Sidetone mixer */
static const struct snd_kcontrol_new max9867_left_sidetone_mixer_controls[] = {
	SOC_DAPM_SINGLE("Sidetone Switch", MAX9867_SIDETONE, 6, 1, 0),
};

static const struct snd_kcontrol_new max9867_right_sidetone_mixer_controls[] = {
	SOC_DAPM_SINGLE("Sidetone Switch", MAX9867_SIDETONE, 7, 1, 0),
};

/* Line out switch */
static const struct snd_kcontrol_new max9867_left_line_out_control =
	SOC_DAPM_SINGLE("Switch", MAX9867_LEFTVOL, 6, 1, 1);

static const struct snd_kcontrol_new max9867_right_line_out_control =
	SOC_DAPM_SINGLE("Switch", MAX9867_RIGHTVOL, 6, 1, 1);
...
static const struct snd_soc_dapm_widget max9867_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MIXER_NAMED_CTL("Left Input Mixer", SND_SOC_NOPM, 0, 0,
			max9867_left_input_mixer_controls,
			ARRAY_SIZE(max9867_left_input_mixer_controls)),
	SND_SOC_DAPM_MIXER_NAMED_CTL("Right Input Mixer", SND_SOC_NOPM, 0, 0,
			max9867_right_input_mixer_controls,
			ARRAY_SIZE(max9867_right_input_mixer_controls)),
	...
	SND_SOC_DAPM_MIXER("Digital Left", SND_SOC_NOPM, 0, 0,
			max9867_left_sidetone_mixer_controls,
			ARRAY_SIZE(max9867_left_sidetone_mixer_controls)),
	SND_SOC_DAPM_MIXER("Digital Right", SND_SOC_NOPM, 0, 0,
			max9867_right_sidetone_mixer_controls,
			ARRAY_SIZE(max9867_right_sidetone_mixer_controls)),

	SND_SOC_DAPM_MIXER_NAMED_CTL("Left Output Mixer", SND_SOC_NOPM, 0, 0,
			max9867_left_output_mixer_controls,
			ARRAY_SIZE(max9867_left_output_mixer_controls)),
	SND_SOC_DAPM_MIXER_NAMED_CTL("Right Output Mixer", SND_SOC_NOPM, 0, 0,
			max9867_right_output_mixer_controls,
			ARRAY_SIZE(max9867_right_output_mixer_controls)),
	SND_SOC_DAPM_DAC("DACL", "HiFi Playback", SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_DAC("DACR", "HiFi Playback", SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_SWITCH("Master Playback Left", SND_SOC_NOPM, 0, 0,
			&max9867_left_line_out_control),
	SND_SOC_DAPM_SWITCH("Master Playback Right", SND_SOC_NOPM, 0, 0,
			&max9867_right_line_out_control),
...
static const struct snd_soc_dapm_route max9867_audio_map[] = {
	...
	{"Left Input Mixer",  "Mic Left Capture Switch",  "MICL"},
	{"Right Input Mixer", "Mic Right Capture Switch", "MICR"},
	{"Left Input Mixer",  "Line Left Capture Switch",  "Left Line Input"},
	{"Right Input Mixer", "Line Right Capture Switch", "Right Line Input"},
	{"DMICL Mux", "DMIC", "DMICL"},
	{"DMICR Mux", "DMIC", "DMICR"},
	{"DMICL Mux", "ADC", "Left Input Mixer"},
	{"DMICR Mux", "ADC", "Right Input Mixer"},
	{"ADCL", NULL, "DMICL Mux"},
	{"ADCR", NULL, "DMICR Mux"},

	{"Digital Left",  "Sidetone Switch", "ADCL"},
	{"Digital Right", "Sidetone Switch", "ADCR"},
	{"DACL", NULL, "Digital Left"},
	{"DACR", NULL, "Digital Right"},

	{"Left Output Mixer",  "Line Left Bypass Switch",  "Left Line Input"},
	{"Right Output Mixer", "Line Right Bypass Switch", "Right Line Input"},
	{"Left Output Mixer",  NULL, "DACL"},
	{"Right Output Mixer", NULL, "DACR"},
	{"Master Playback Left",  "Switch", "Left Output Mixer"},
	{"Master Playback Right", "Switch", "Right Output Mixer"},
	{"LOUT", NULL, "Master Playback Left"},
	{"ROUT", NULL, "Master Playback Right"},
};
...
static void max9867_jack_work(struct work_struct *work) {
	struct max9867_priv *max9867 = container_of(work,
		struct max9867_priv,
		jack_work.work);
	struct snd_soc_codec *codec = max9867->codec;
	int ret, reg;

	ret = regmap_read(max9867->regmap, MAX9867_JACKSTATUS, &reg);
	if (ret != 0) {
		dev_err(codec->dev, "failed to read MAX9867_JACKSTATUS: %d\n", ret);
		return;
	}

	switch (reg & (MAX9867_LSNS_MASK | MAX9867_JKSNS_MASK | MAX9867_JKMIC_MASK)) {
	case MAX9867_JKSNS_MASK | MAX9867_JKMIC_MASK:
		dev_dbg(codec->dev, "No Headset Detected\n");

		max9867->jack_status = 0;

		break;
	case 0:
		dev_dbg(codec->dev, "Headphone Detected\n");

		max9867->jack_status = SND_JACK_HEADPHONE;

		break;
	case MAX9867_JKSNS_MASK:
		dev_dbg(codec->dev, "Microphone Detected\n");

		max9867->jack_status = SND_JACK_MICROPHONE;

		break;
	default:
		dev_info(codec->dev, "Unrecognized Jack Status: %d\n", reg);
		return;
	}

	snd_soc_jack_report(max9867->jack, max9867->jack_status, SND_JACK_HEADSET);
}

static irqreturn_t max9867_irq(int irq, void *data)
{
	struct max9867_priv *max9867 = data;
	struct snd_soc_codec *codec = max9867->codec;
  int ret;
	...
	dev_dbg(codec->dev, "max9867 irq\n");

	ret = regmap_read(max9867->regmap, MAX9867_INTEN, &mask);
	if (ret != 0) {
		dev_err(codec->dev, "failed to read MAX9867_INTEN: %d\n", ret);
		return IRQ_NONE;
	}

	ret = regmap_read(max9867->regmap, MAX9867_STATUS, &active);
	if (ret != 0) {
		dev_err(codec->dev, "failed to read MAX9867_STATUS: %d\n", ret);
		return IRQ_NONE;
	}
	...
	if (active & MAX9867_CLD_MASK)
		dev_dbg(codec->dev, "MAX9867_CLD_MASK\n");

	if (active & MAX9867_SLD_MASK)
		dev_dbg(codec->dev, "MAX9867_SLD_MASK\n");

	if (active & MAX9867_ULK_MASK) {
		dev_dbg(codec->dev, "MAX9867_ULK_MASK\n");
	}

	if (active & MAX9867_JDET_MASK) {
		dev_dbg(codec->dev, "MAX9867_JDET_MASK\n");
                            ...
                            struct snd_soc_jack *jack)
{
	...
	if (jack) {
		regmap_update_bits(max9867->regmap, MAX9867_INTEN,
			MAX9867_IJDET_MASK, MAX9867_IJDET_MASK);
	} else {
		regmap_update_bits(max9867->regmap, MAX9867_INTEN,
			MAX9867_IJDET_MASK, 0);
	}

	/* Send an initial empty report */
	snd_soc_jack_report(max9867->jack, 0, SND_JACK_HEADSET);
...
EXPORT_SYMBOL_GPL(max9867_set_jack_detect);
...
static int max9867_probe(struct snd_soc_codec *codec)
{
	...
	INIT_DELAYED_WORK(&max9867->jack_work, max9867_jack_work);
	return 0;
}

static int max9867_remove(struct snd_soc_codec *codec)
{
	struct max9867_priv *max9867 = snd_soc_codec_get_drvdata(codec);

	cancel_delayed_work_sync(&max9867->jack_work);
...
static int max9867_set_bias_level(struct snd_soc_codec *codec,
		...
		const struct i2c_device_id *id)
{
  ...
  /* jack detects */
	...
	jack_gpio = of_get_gpio(i2c->dev.of_node, 0);
	...
	if (i2c->irq) {
		ret = devm_request_threaded_irq(&i2c->dev, i2c->irq, NULL,
			max9867_irq, IRQF_TRIGGER_LOW | IRQF_ONESHOT,
			"max9867", max9867);
		if (ret)
			dev_err(&i2c->dev, "Failed to reguest IRQ: %d\n", ret);

		/* Enable jack detection */
		regmap_update_bits(max9867->regmap, MAX9867_MODECONFIG,
			MAX9867_JDETEN_MASK, MAX9867_JDETEN_MASK);
	}
