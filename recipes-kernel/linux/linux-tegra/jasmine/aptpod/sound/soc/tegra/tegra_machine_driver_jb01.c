...
#define PARAMS(sformat, channels)                                              \
  {                                                                            \
    .formats = sformat, .rate_min = 48000, .rate_max = 48000,                  \
...
static const char *const tegra_machine_srate_text[] = {
    "None",  "8kHz",  "16kHz", "44kHz", "48kHz",  "11kHz",  "22kHz",
    "24kHz", "32kHz", "88kHz", "96kHz", "176kHz", "192kHz",
};
...
static const struct soc_enum tegra_machine_codec_rate = SOC_ENUM_SINGLE_EXT(
    ARRAY_SIZE(tegra_machine_srate_text), tegra_machine_srate_text);

static const struct soc_enum tegra_machine_codec_format = SOC_ENUM_SINGLE_EXT(
    ARRAY_SIZE(tegra_machine_format_text), tegra_machine_format_text);

static const struct soc_enum tegra_machine_jack_state = SOC_ENUM_SINGLE_EXT(
    ARRAY_SIZE(tegra_machine_jack_state_text), tegra_machine_jack_state_text);

static const int tegra_machine_srate_values[] = {
    0,     8000,  16000, 44100, 48000,  11025,  22050,
    24000, 32000, 88200, 96000, 176400, 192000,
};
...
static const struct snd_soc_dapm_widget tegra_machine_dapm_widgets[] = {
    ...
    SND_SOC_DAPM_HP("y Headphone Jack", NULL),
    SND_SOC_DAPM_MIC("y Int Mic", NULL),
    SND_SOC_DAPM_MIC("y Mic Jack", NULL),
...
static struct snd_soc_pcm_stream tegra_machine_asrc_link_params[] = {
    PARAMS(SNDRV_PCM_FMTBIT_S32_LE, 8), PARAMS(SNDRV_PCM_FMTBIT_S16_LE, 2),
    PARAMS(SNDRV_PCM_FMTBIT_S16_LE, 2), PARAMS(SNDRV_PCM_FMTBIT_S16_LE, 2),
    PARAMS(SNDRV_PCM_FMTBIT_S16_LE, 2), PARAMS(SNDRV_PCM_FMTBIT_S16_LE, 2),
};
...
static int
tegra_machine_codec_get_jack_state(struct snd_kcontrol *kcontrol,
                                   struct snd_ctl_elem_value *ucontrol) {
...
static int tegra_machine_set_params(struct snd_soc_card *card,
  ...
  int num_of_dai_links =
      machine->soc_data->num_ahub_links + machine->num_codec_links;
...
static int tegra_machine_dai_init(struct snd_soc_pcm_runtime *runtime,
  ...
  err = tegra_alt_asoc_utils_set_rate(&machine->audio_clock, srate, channels,
                                      sample_size, 0, 0);
  if (err < 0) {
  ...
  if (rtd) {
    ...
    err = snd_soc_dai_set_sysclk(rtd->codec_dai, RT5659_SCLK_S_MCLK, aud_mclk,
                                 SND_SOC_CLOCK_IN);
    if (err < 0) {
  ...
  if (rtd) {
    if (!strcmp(rtd->codec_dai->name, "tas2552-amplifier")) {
      err = snd_soc_dai_set_sysclk(rtd->codec_dai, TAS2552_PDM_CLK_IVCLKIN,
  ...
  rtd = snd_soc_get_pcm_runtime(card, "max9867-codec");
  if (rtd) {
    dai_params = (struct snd_soc_pcm_stream *)rtd->dai_link->params;
    ...
    err = snd_soc_dai_set_sysclk(rtd->codec_dai, MAX9867_SYSCLK, aud_mclk,
                                 SND_SOC_CLOCK_IN);
    if (err) {
      dev_err(card->dev, "failed to set max9867 sysclk!\n");
      return err;
    }
  }
...
static int tegra_machine_compr_set_params(struct snd_compr_stream *cstream) {
  ...
  if (platform->driver->compr_ops && platform->driver->compr_ops->get_params) {
    err = platform->driver->compr_ops->get_params(cstream, &codec_params);
    if (err < 0) {
  ...
  err = tegra_machine_dai_init(rtd, codec_params.sample_rate,
                               codec_params.ch_out, SNDRV_PCM_FORMAT_S16_LE);
  if (err < 0) {
    dev_err(card->dev, "Failed dai init\n");
    return err;
  }
...
static int tegra_machine_respeaker_init(struct snd_soc_pcm_runtime *rtd) {
   ...
   * this callback
   */
  err = snd_soc_dai_set_sysclk(rtd->codec_dai, 0, 24000000, SND_SOC_CLOCK_IN);
  if (err) {
...
static int tegra_machine_max9867_init(struct snd_soc_pcm_runtime *rtd) {
  ...
  err = max9867_set_jack_detect(rtd->codec, &tegra_machine_hp_jack);
  if (err) {
    dev_err(card->dev, "Failed to set jack for MAX9867: %d\n", err);
    return err;
  }
...
static int tegra_machine_rt565x_init(struct snd_soc_pcm_runtime *rtd) {
  ...
  err = snd_soc_card_jack_new(card, "Headset Jack", SND_JACK_HEADSET, jack,
                              NULL, 0);
  if (err) {
    dev_err(card->dev, "Headset Jack creation failed %d\n", err);
    return err;
...
static int codec_init(struct tegra_machine *machine) {
  ...
  for (i = 0; i < num_links; i++) {
    ...
    else if (strstr(dai_links[i].name, "max9867-codec"))
...
static void set_dai_ops(struct tegra_machine *machine) {
  ...
  for (i = machine->soc_data->adsp_compr_dai_link_start;
    ...
    machine->asoc->dai_links[i].compr_ops = &tegra_machine_compr_ops;
#endif
  ...
  if (!(of_machine_is_compatible("nvidia,tegra210") ||
    ...
    for (i = 0; i < 6; i++) {
      ...
      machine->asoc->dai_links[tx].params = &tegra_machine_asrc_link_params[i];
      machine->asoc->dai_links[rx].params = &tegra_machine_asrc_link_params[i];
    }
  }
#endif
}
...
static int add_dai_links(struct platform_device *pdev) {
  ...
  machine->asoc = devm_kzalloc(&pdev->dev, sizeof(*machine->asoc), GFP_KERNEL);
  if (!machine->asoc)
    return -ENOMEM;
...
static const struct snd_kcontrol_new tegra_machine_controls[] = {
    ...
    SOC_ENUM_EXT("codec-x format", tegra_machine_codec_format,
                 tegra_machine_codec_get_format,
                 tegra_machine_codec_put_format),
    {
        .access = SNDRV_CTL_ELEM_ACCESS_READ,
        // SOC_ENUM_EXT("y Jack-state", tegra_machine_jack_state,
        // tegra_machine_codec_get_jack_state, NULL),
...
static const struct of_device_id tegra_machine_of_match[] = {
    ...
    {.compatible = "nvidia,tegra-audio-mystique", .data = &soc_data_tegra186},
...
static int tegra_machine_driver_probe(struct platform_device *pdev) {
  ...
  if (of_property_read_u32(np, "mclk-fs", &machine->audio_clock.mclk_scale) < 0)
  ...
  ret = tegra_alt_asoc_utils_init(&machine->audio_clock, &pdev->dev, card);
  if (ret < 0)
    goto cleanup_asoc;
  ...
  tegra_machine_add_i2s_codec_controls(card, machine->soc_data->num_ahub_links +
